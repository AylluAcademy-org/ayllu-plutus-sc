...
mkPolicyFT :: TokenName -> L.PaymentPubKeyHash -> () -> PlutusV2.ScriptContext -> Bool
mkPolicyFT tn pkh () ctx = traceIfFalse "unauthorized signature" validSignature &&
  ...
  where
    ...
    validSignature :: Bool
    validSignature = Contexts.txSignedBy info $ L.unPaymentPubKeyHash pkh

    validTokenName :: Bool
    validTokenName = case flattenValue (Contexts.txInfoMint info) of
      [(_, tn', _)] -> tn' == tn
      _             -> False
...
mkPolicyNFT :: Tx.TxOutRef -> TokenName -> () -> PlutusV2.ScriptContext -> Bool
mkPolicyNFT oref tn () ctx = traceIfFalse "UTxO not consumed"   hasUTxO           &&
  ...
  where
    ...
    hasUTxO :: Bool
    hasUTxO = any (\i -> Contexts.txInInfoOutRef i == oref) $ Contexts.txInfoInputs info

    checkMintedAmount :: Bool
    checkMintedAmount = case flattenValue (Contexts.txInfoMint info) of
...
{-# INLINABLE mkVault1 #-}
mkVault1 :: L.PaymentPubKeyHash -> () -> () -> PlutusV2.ScriptContext -> Bool
...
{-# INLINABLE mkVault2 #-}
mkVault2 :: ValidationDatum -> () -> PlutusV2.ScriptContext -> Bool
mkVault2 cd _ ctx = traceIfFalse "NFT validation failed" validateNFT
  where
    ...
    validateNFT :: Bool
    validateNFT = any isNftTk $ Contexts.txInfoInputs info
      where
        isNftTk = any (== nftCs) . symbols . Contexts.txOutValue . Contexts.txInInfoResolved
